...
// This API route acts as a proxy for our historical trend data.
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const apiUrl = `${process.env.NEXT_PUBLIC_ANALYTICS_API_URL}/api/summary/historical-trends/${endDate}`;
    
    const backendRes = await fetch(apiUrl);

    if (!backendRes.ok) {
      const errorData = await backendRes.text();
      console.error("Backend API Error (trends):", errorData);
    ...
    console.error("Proxy API Error (trends):", error);
