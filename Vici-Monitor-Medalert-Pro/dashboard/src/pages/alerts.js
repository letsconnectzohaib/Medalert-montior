...
async function fetchAlerts(baseUrl, token, { shiftDate, status, severity, limit } = {}) {
  const qs = new URLSearchParams();
  if (shiftDate) qs.set('shiftDate', shiftDate);
  if (status) qs.set('status', status);
  if (severity) qs.set('severity', severity);
  if (limit) qs.set('limit', String(limit));
  const res = await fetch(`${normalizeBaseUrl(baseUrl)}/api/alerts?${qs.toString()}`, {
...
async function alertAction(baseUrl, token, id, action) {
  const res = await fetch(`${normalizeBaseUrl(baseUrl)}/api/alerts/${encodeURIComponent(String(id))}/action`, {
    method: 'POST',
...
async function refresh(state) {
  ...
  const table = el('table', {}, [
    ...
    el('tbody', {}, r.alerts.map((a) => row(state, a)))
  ]);
