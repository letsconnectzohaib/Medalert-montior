...
import {
  ...
  Pause, AlertTriangle, Activity, RefreshCw, TrendingUp, TrendingDown, Zap
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const { latestSnapshot, chartData, lastRefresh, refresh, isLoading, error } = useAutoRefresh(15); // Reduced to 15 seconds for more frequent updates
  const slaTracker = useSLATracker(20);

  // Fix summary access with proper typing
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground font-mono">
            Real-Time Monitoring • Live at {now}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={`Last refresh: ${format(lastRefresh, "HH:mm:ss.SSS")}`}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        <MetricCard title="Active Calls" value={summary?.activeCalls || 0} icon={PhoneCall} variant="primary"
          subtitle={<MiniSparkline data={chartData.agents.map(d => d.calls)} color="hsl(187, 80%, 48%)" />} />
        <MetricCard title="Waiting Calls" value={summary?.waitingCalls || 0} icon={Clock} variant={(summary?.waitingCalls || 0) >= 5 ? "warning" : "default"}
          subtitle={<MiniSparkline data={chartData.calls.map(d => d.duration)} color="hsl(38, 92%, 50%)" />} />
        <MetricCard title="Agents Logged In" value={summary?.agentsLoggedIn || 0} icon={Users} variant="success" />
        <MetricCard title="Agents In Calls" value={summary?.agentsInCalls || 0} icon={UserCheck} variant="primary" />
        <MetricCard title="Agents Paused" value={summary?.agentsPaused || 0} icon={Pause} variant={(summary?.agentsPaused || 0) > 3 ? "warning" : "default"} />
      </div>

      {/* KPI Row 2 */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <MetricCard title="Agents Waiting" value={summary?.agentsWaiting || 0} icon={UserX} />
        <MetricCard title="Agents Dispo" value={summary?.agentsDispo || 0} icon={PhoneOff} />
        <MetricCard title="Ringing" value={summary?.ringingCalls || 0} icon={PhoneCall} />
        <MetricCard title="Agents Dead" value={summary?.agentsDead || 0} icon={AlertTriangle} variant={(summary?.agentsDead || 0) > 0 ? "destructive" : "default"} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <SLAGauge sla={{ 
          current: slaTracker.avgWaitTime || 0, 
          targetSeconds: 20, 
          percentage: Math.max(0, 100 - ((slaTracker.avgWaitTime || 0) / 20) * 100) 
        }} />
        <div className="lg:col-span-3">
          <CallVolumeChart data={[latestSnapshot]} />
        </div>
      </div>
