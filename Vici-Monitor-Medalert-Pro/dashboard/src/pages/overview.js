...
function patchSpark(svgId, pathId, dotsId, points, stroke) {
  ...
  for (let i = 0; i < Math.min(children.length, pts.length); i++) {
    const c = children[i];
    const p = pts[i];
    if (c.getAttribute("cx") !== p.x.toFixed(1))
      c.setAttribute("cx", p.x.toFixed(1));
    if (c.getAttribute("cy") !== p.y.toFixed(1))
...
function patchTrends(state) {
  ...
  setText(
    "ov_tr_note",
    `Last ${win} minutes • ${pts.length} points (in-memory)`,
  );
}
...
export function renderOverview(state) {
  ...
  const summary = el("section", { class: "card" }, [
    ...
    el("div", { id: "ov_lastUpdate", class: "note" }, [
      `Last update: ${snap.timestamp || "—"} • WS: ${state.wsStatus}`,
    ]),
  ]);
  ...
  const meta = el("section", { class: "card wide" }, [
    ...
    el("div", { class: "metaGrid" }, [
      ...
      kv(
        "Dropped/Answered",
        `${m?.droppedAnswered?.dropped ?? 0}/${m?.droppedAnswered?.answered ?? 0}`,
        "ov_droppedAnswered",
      ),
  ...
  const buckets = el("section", { class: "card wide" }, [
    ...
    el(
      ...
      BUCKET_LABELS.map(([bucket, label]) =>
        bucketCard(label, counts[bucket] || 0, `ov_bucket_${bucket}`),
      ),
    ),
    el("div", { class: "note" }, [
      "Counts computed from snapshot.agents[].stateBucket.",
    ]),
  ]);
  ...
  return el("div", { id: "ov_root", class: "grid" }, [
    summary,
    meta,
    trends,
    buckets,
